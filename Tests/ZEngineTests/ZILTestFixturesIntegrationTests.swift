...
struct ZILTestFixturesIntegration {
    ...
    func stepByStepAssemblerDebugTest() throws {
        print("=== Testing MemoryLayoutManager step by step ===")

        // Test 1: Basic initialization
        print("1. Creating MemoryLayoutManager...")
        let layoutManager = MemoryLayoutManager(version: .v3)
        print("   ✓ MemoryLayoutManager created successfully")

        // Test 2: Try to access basic properties
        print("2. Testing basic properties...")
        // Note: These methods might not be public, so we'll test what we can

        // Test 3: Try generateHeader specifically
        print("3. Testing individual generation methods...")

        do {
            print("4. Attempting generateStoryFile()...")
            let storyFile = try layoutManager.generateStoryFile()
            print("   ✓ generateStoryFile succeeded: \(storyFile.count) bytes")
            #expect(storyFile.count >= 64)
        } catch {
            print("   ✗ generateStoryFile failed: \(error)")
            throw error
        }
    }
    ...
    func vmLoadingTestWithAssembledBytecode() throws {
        ...
        print("=== VM Loading Test ===")
        print("Generated bytecode size: \(bytecode.count) bytes")
        ...
        print("File written to disk: \(storyFile.path)")
        print("File size on disk: \(fileSize?.intValue ?? -1) bytes")
    ...
    func zilToZapToBytcodePipelineIntegrity() throws {
        ...
        5. VM Loading: ✓ Version \(vm.version.rawValue)
